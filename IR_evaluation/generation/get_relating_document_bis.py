...
def _load_run_file_index(run_file_path: str) -> Dict[str, Dict[str, Any]]:
    """
    Index a run JSONL file by question_id.
    Cached by file mtime to avoid re-reading.
    """
    abspath = os.path.abspath(run_file_path)
    mtime = os.path.getmtime(abspath)

    cached = _RUN_FILE_CACHE.get(abspath)
    ...
    _RUN_FILE_CACHE[abspath] = {"mtime": mtime, "by_qid": by_qid}
    return by_qid
...
def get_relating_documents_bis(
    ...
    *,
    ...
    run_file_path: Optional[str] = None,     # required if source="run_file"
    ...
    #     ids, docs, metas, dists, embs = _dedup_by_similarity(ids, docs, metas, dists, embs, threshold=0.9, verbose=verbose)
    # else:
    ...
    t = time.perf_counter()
    if cfg.model_name == "HNSW":
        scores = [-d if d is not None else float('-inf') for d in dists]
        spans = [None] * len(docs)
    else:
        scores, spans = rerank_with_spans(query, docs, config=cfg)
