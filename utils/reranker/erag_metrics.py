...
EVALUATION_JUDGE_MODEL = os.getenv("JUDGE_MODEL", "gpt-4.1-mini")
...
def chunk_relevance(
    ...
    chunk: str,
    gold_answer: str,
    llm_model: str = EVALUATION_JUDGE_MODEL,
) -> Dict[str, Any]:
    if not chunk or not chunk.strip():
        return {
            ...
            "chunk_answer": "",
            "relevance_score": 0.0,
            "justification": "Empty chunk",
        }

    judge_prompt = f"""You are a STRICT evaluator of whether a retrieved document chunk explicitly supports a reference answer.
...
- Score ONLY what is explicitly supported by the chunk.
- Do NOT reward plausible inference.
- Do NOT assume missing dates, names, places, or causal links.
- OCR noise should be tolerated only when the intended fact remains clearly recoverable.
- A chunk that is merely on-topic is NOT relevant enough.

SCORING:
- 1.0 = The chunk explicitly states the key fact(s) needed for the reference answer.
- 0.8 = The chunk explicitly supports most key facts, with only minor missing detail.
- 0.6 = The chunk explicitly supports an important part of the reference answer.
- 0.4 = The chunk contains weak or incomplete explicit evidence, but still materially helps.
- 0.2 = The chunk is topically related but does not support the answer enough.
- 0.0 = Irrelevant, misleading, contradictory, or only inferentially related.

IMPORTANT:
- If the chunk does not explicitly support the answer, prefer 0.0 or 0.2.
- Be conservative.
- Do not be generous.
...
{{"relevance_score": <float 0-1>, "justification": "<max 8 words>"}}"""
    ...
    try:
        ...
        justification = str(result.get("justification", ""))[:50]
        ...
        return {
            ...
            "chunk_answer": "",
            "relevance_score": 0.0,
            "justification": f"Judge error: {type(e).__name__}",
        }
...
def compute_single_erag(
    ...
    # Handle edge cases
    if k == 0:
        print(f"[WARNING] No retrieved chunks for question: {question[:120]}")
        return {
    ...
    if not gold_answer or not str(gold_answer).strip():
        print("\n[WARNING] Missing or empty gold_answer")
        print(f"Question: {question[:200]}")
        print(f"Gold answer: {repr(gold_answer)}")
        return {
            "chunk_relevance_precision_at_k": 0.0,
            "chunk_relevance_ndcg_at_k": 0.0,
            "chunk_details": []
...
def compute_erag_metrics(
    ...
    print("\n" + "=" * 70)
    print("eRAG CONFIGURATION")
    print("=" * 70)
    print(f"Judge model (LJCC): {EVALUATION_JUDGE_MODEL}")
    print(f"Threshold tau: {threshold_tau}")
    print("=" * 70)
    ...
    def process_sample(idx: int, sample: Dict) -> tuple:
        ...
        metrics = compute_single_erag(
            ...
            llm_model=EVALUATION_JUDGE_MODEL,
        )
        progress["count"] += 1
...
def compute_single_erag_embedding(
    ...
    if not gold_answer or not str(gold_answer).strip():
        print("\n[WARNING] Missing or empty gold_answer (embedding mode)")
        print(f"Question: {question[:200]}")
        print(f"Gold answer: {repr(gold_answer)}")
        return {"erag_precision_at_k": 0.0, "erag_ndcg_at_k": 0.0}
